"""
evaluation/plot_detailed_deeplabv3plus_eval.py

Generate plots from detailed DeepLabV3+ IDD-AW evaluation CSV files.

Inputs:
1. IDD-AW trained DeepLabV3+ detailed evaluation
2. Clean IDD trained DeepLabV3+ tested on IDD-AW detailed evaluation
...
python -m dissertation.evaluation.plot_detailed_deeplabv3plus_eval \
    --iddaw_dir dissertation/results/deeplabv3plus/detailed_eval \
    --cross_domain_dir dissertation/results/cross_domain/deeplabv3plus_idd_clean_to_iddaw/detailed_eval \
    --output_dir dissertation/results/summary/detailed_deeplabv3plus_eval
"""
...
def plot_weather_miou_comparison(iddaw_df, clean_to_aw_df, output_dir):
    ...
    plt.ylabel("mIoU")
    plt.title("DeepLabV3+: Weather-wise mIoU on IDD-AW")
...
def plot_weather_pixel_accuracy_comparison(iddaw_df, clean_to_aw_df, output_dir):
    ...
    plt.title("DeepLabV3+: Weather-wise Pixel Accuracy on IDD-AW")
    plt.xticks(x, weather_order)
    plt.ylim(0, 1.0)
...
def plot_safety_iou_overall_comparison(iddaw_df, clean_to_aw_df, output_dir):
    ...
    plt.ylabel("IoU")
    plt.title("DeepLabV3+: Safety-critical Class IoU on IDD-AW")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate DeepLabV3+ detailed evaluation plots"
    )

    parser.add_argument(
        "--iddaw_dir",
        default="dissertation/results/deeplabv3plus/detailed_eval",
        help="Detailed evaluation folder for IDD-AW trained DeepLabV3+",
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "deeplabv3plus_idd_clean_to_iddaw/detailed_eval"
        ),
        help="Detailed evaluation folder for clean IDD trained DeepLabV3+ tested on IDD-AW",
    )

    parser.add_argument(
        "--output_dir",
        default="dissertation/results/summary/detailed_deeplabv3plus_eval",
    ...
    plot_safety_iou_heatmap(
        iddaw_safety_df,
        "DeepLabV3+: Safety-critical IoU by Weather — Trained on IDD-AW",
        output_dir / "safety_iou_heatmap_iddaw_trained.png",
    )

    plot_safety_iou_heatmap(
        clean_to_aw_safety_df,
        "DeepLabV3+: Safety-critical IoU by Weather — Trained on IDD Clean",
        output_dir / "safety_iou_heatmap_clean_trained_on_iddaw.png",
    )

    print("\nAll detailed DeepLabV3+ evaluation plots generated successfully.")
